...
def normalize(rows: list[dict[str, str]], input_file: str = "") -> NormalizationResult:
    ...
    """
    return normalize_rows(rows, input_file=input_file)


def normalize_file(path: str, input_file: str = "") -> NormalizationResult:
    """Read and normalize a file. Auto-detects CSV, FHIR JSON, HL7, C-CDA, Excel."""
    from pathlib import Path
    p = Path(path)
    rows = read_input(p)
    return normalize_rows(rows, input_file=input_file or p.name)
