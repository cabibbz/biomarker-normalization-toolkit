...
- **Plausibility checks** across the supported catalog
- **REST API surface** with `/v1/` versioning, rate limiting, and Prometheus `/metrics`
- **LOINC code lookup**: test names that are LOINC codes resolve directly
- **Docker**: multi-stage build, non-root user, healthcheck
- JSON output includes `bnt_version`, `generated_at`, and `schema_version: 0.2.0`
- **Apache-2.0 open-source distribution** with full-access API and CLI behavior
- **Community project surface**: OSS docs, contribution policy, and public maintainer files
- **Release metadata hardening**: package URLs, CODEOWNERS, Dependabot, examples, OpenAPI drift check in CI, and citation metadata
- **Validation transparency**: tracked public interop fixtures plus wheel, sdist, packaged-server, and public-fixture sanity checks

### Fixed
- PhenoAge glucose formula (was `/18`, now `ln(glucose)` per Levine 2018)
- Upload endpoint parity with the JSON endpoint
- HL7 specimen leak, UCUM round-trip, European decimals, and FHIR `effectiveDateTime`
- Sibling redirect now marked (`confidence=medium`, `reason=sibling_unit_redirect`)
- Content-length crash, CORS wildcard default, and rate limiter memory usage
- Removed stale gated-product scripts and local-only fixture assumptions from the public repository surface

### Validated
- repository test suite, tracked parser fixtures, and OpenAPI drift checks
- wheel and source-distribution install smoke tests
- packaged REST server startup and live HTTP smoke paths
...
- Blood pH, Fibrinogen, and eAG biomarkers (74 total)
- HL7 specimen extraction from `OBR-15` and `SPM-4`
...
- Metabolic, lipid, renal, liver, thyroid, inflammation, CBC, WBC differential, coagulation, electrolytes, vitamins, minerals, cardiac, blood gas, and urinalysis
...
- `bnt analyze --input <file>` - coverage gap report for dataset review
...
- Added inequality value detection (`">100"` gets a specific reason code)
- Added duplicate `source_row_id` warning in output
- Corrected LOINC codes for BUN and Iron

### Infrastructure
- Dockerfile for self-hosted Docker deployment
- UCUM code mapping for all FHIR unit codes
- 57 automated tests
- External FHIR bundle validation
- Sample data pipeline with converters for MIMIC-IV, Synthea, and HAPI FHIR
